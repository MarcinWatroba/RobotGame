...
void Robot::forward(unsigned int part, const float z)
{
	...
	std::cout << _limbAngle << std::endl;
	q_rotation[1] = _xRotation2 * q_rotation[1];
	//q_limbRotation = _yRotation * q_limbRotation;

	q_limbRotation[1] = q_limbRotation[1] * _xRotation2;
		//std::cout << "RIGHT " << _xRotation2.x << std::endl;
	...
	//std::cout << "body: " << _rotation[1][2] << std::endl;
	//std	_lHandPosition += _zaxis * z;
...
void Robot::setVertices(vector<float>& vertices)
{
	_vertices = vertices;
}

void Robot::setNormals(vector<float>& normals)
{
	_normals = normals;
}

void Robot::VBOobject()
{
